...
tweet.score = function(text, posWords, negWords ){
    textPosWords.match  = match(posWords,text)
    textPosWords.match = !is.na(textPosWords.match)
    textNegWords.match = match(negWords, text)
    textNegWords.match = !is.na(textNegWords.match)
    
    score = sum(textPosWords.match) - sum(textNegWords.match)
    return(score)
}

#lapply(iphoneTweets, function(tweet) tweet.score(tweet, posWords , negWords))
