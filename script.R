...
#   
# }
Tweets.df = data.frame(text = get("content", iphoneTweetsClean))
...
IphoneTweetsScore = lapply(iphoneTweetsClean, function(tweet) tweet.score(tweet, posWords , negWords))
print(Tweets.df)

IphoneTweetsScore.df <- data.frame(matrix(unlist(IphoneTweetsScore)))

Tweets.df["score"]<- IphoneTweetsScore.df
print(Tweets.df)
